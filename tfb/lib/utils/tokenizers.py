...
def str_to_tokens_pad_max_len(str_array, tokenizer, max_len, use_sep=True):
    tokens = [
        tokenizer.encode(x, use_sep) for x in str_array
    ]
    tokens[0] = tokens[0] + [0] * (max_len - len(tokens[0]))
